...
This script builds a warehouse environment with multiple autonomous mobile
robots (AMRs) and runs a timeline-gated simulation loop.
"""
...
def main():
    ...
    add_reference_with_pose(
        ...
        position=np.array([16.23, -9.67, 0.06]),
        orientation=euler_xyz_to_quat_wxyz(
            orientation=np.array([0.0, 0.0, 0.0]), degrees=True
        ),
    )

    add_reference_with_pose(
        asset_path="/Isaac/Robots/Idealworks/iwhub/iw_hub.usd",
        prim_path="/World/iw_hub",
        position=np.array([15.38, -11.79, 0.08]),
        orientation=euler_xyz_to_quat_wxyz(
            orientation=np.array([0.0, 0.0, 90.0]), degrees=True
        ),
    )
