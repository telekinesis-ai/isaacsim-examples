...
This script builds a warehouse environment with multiple hunmanoid robots and
runs a timeline-gated simulation loop.
"""
...
def main():
    ...
    # Add humanoid to scene and set pose.
    add_reference_with_pose(
        asset_path="/Isaac/Robots/Unitree/G1/g1.usd",
        prim_path="/World/g1",
        position=np.array([6.85, -10.92, 0.8]),
    ...
    add_reference_with_pose(
        asset_path="/Isaac/Robots/XiaoPeng/PX5/px5.usd",
        prim_path="/World/px5",
        position=np.array([7.0, -13.0, 0.8]),
        orientation=euler_xyz_to_quat_wxyz(
            orientation=np.array([0.0, 0.0, 90.0]), degrees=True
        ),
    )
    ...
    add_reference_with_pose(
        asset_path="/Isaac/Robots/FourierIntelligence/GR-1/GR1T2_fourier_hand_6dof/GR1T2_fourier_hand_6dof.usd",
        prim_path="/World/gr1",
        position=np.array([5.26, -9.4, 0.95]),
        orientation=euler_xyz_to_quat_wxyz(
            orientation=np.array([0.0, 0.0, 90.0]), degrees=True
        ),
    )
