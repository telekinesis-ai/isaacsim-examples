...
def main():
    ...
    High-level flow:
        1. Create `/World` and environment assets.
    ...
    stand_usd_path = (
        assets_root_path
        + "/Isaac/Samples/Leonardo/Stage/ur10_bin_stacking_long_conveyor.usd"
    )
    ...
    # Wrapper object name for debug readability in object registry.
    xfrom_name = "ur10_with_conveyor_xform"
    xform = world.scene.add(
        SingleXFormPrim(ur10_with_conveyor_prim_path, name=xfrom_name)
    )
    xform.set_world_pose(
    ...
    gripper_wrapper_path = (
        ur10_with_conveyor_prim_path + "/suction_gripper_mount"
    )
    gripper_prim_path = gripper_wrapper_path + "/suction_gripper"
    ...
    # Add a wrapper XFormPrim for the gripper to facilitate scene-level control and set its local pose relative to the robot flange.
    ...
    assembler.begin_assembly(
        ...
        ur10e_mount_path,
        gripper_prim_path,
        gripper_prim_path,  # gripper attach point is the prim itself since it's a single-prim gripper
        assembly_namespace,
        variant_name,
    )
    assembler.assemble()
