...
Workflow:
    ...
    4. Simulation startup:
        ...
        - Create a single `SingleArticulation` handle for the assembled rig
          (UR10e + RG6 share one articulation after assembly).
        - Run a Play-gated sequence: move the arm, wait for it to settle, then
          close the gripper by driving `finger_joint`.
"""
...
if not ur10e_urdf_path.is_file():
    raise FileNotFoundError(f"Cant load file {ur10e_urdf_path}")
...
# After assembly the RG6 is fixed to the UR10e tool flange, so the whole rig is
# a SINGLE articulation rooted at the UR10e. One SingleArticulation handle drives
# both the 6 arm joints and the gripper's finger_joint (the other 5 RG6 joints
# are URDF mimic joints that follow finger_joint automatically).
robot = SingleArticulation(prim_path=ur10e_prim_path, name="ur10e_with_rg6")
robot.initialize()
...
# Arm target pose (deg -> rad) and gripper closed target (rad, within ~±0.628).
...
# Sequencer state: 0 = command arm, 1 = waiting, 2 = close gripper, 3 = done.
STAGE = 0
wait_counter = 0
...
while simulation_app.is_running():
    ...
    # 1) Move the arm to the target pose.
    if STAGE == 0:
        robot.apply_action(
            ArticulationAction(
    ...
    # 3) Close the gripper.
        ...
        print(f"Commanded finger_joint (rad): {GRIPPER_CLOSE_RAD}")
        STAGE = 3
